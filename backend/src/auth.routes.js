...
async function sendVerificationEmail(to, code) {
  // Cấu hình transporter với Gmail (bạn cần bật "Less secure app access" hoặc dùng App Password)
...
// Lấy Redis instance từ app (đã set ở index.js)
function getRedis(req) {
  return req.app.get('redis');
}

// Xác thực tài khoản qua link
...
router.post('/login', async (req, res) => {
  ...
  try {
    ...
    // Lấy roles, extra_permissions, is_superuser, is_staff
    const payload = {
      user_id: user._id,
      email: user.email,
      roles: user.roles || [],
